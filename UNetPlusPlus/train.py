...
model = UnetPlusPlus(1, False).to(device)
...
if __name__ == '__main__':
    ...
    for epoch in range(n_epochs):
        ...
        if valid_acc > best_acc:
            ...
            torch.save(model.state_dict(), f"UNetPlusPlus/weight/{config['name']}_best.ckpt") # only save best to prevent output memory exceed error
            best_acc = valid_acc
            stale = 0
        else:
            stale += 1
